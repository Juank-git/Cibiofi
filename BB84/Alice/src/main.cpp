...
void performHoming() {
    ...
    Serial.printf("[Alice] Estado inicial sensor Hall: %d\n", initialState);
    ...
    if (!hallTriggered) {
        ...
        Serial.println("[Alice] ERROR: Timeout en búsqueda del sensor Hall");
        stepper.stop();
    ...
    // Fase 3: Ajuste fino
    Serial.println("[Alice] Aproximación fina...");
    ...
    while (digitalRead(HALL_SENSOR_PIN) == HIGH && (millis() - fineAdjustStart < 5000)) {
        ...
        if (stepCount > stepsFor360) {
            Serial.println("[Alice] ERROR: Ajuste fino excedió 1 vuelta completa");
            break;
        }
    }
    
    if (digitalRead(HALL_SENSOR_PIN) == HIGH) {
        Serial.println("[Alice] WARNING: Ajuste fino no encontró posición exacta");
    }
    ...
    if (centralRegistered) {
        ...
        if (result == ESP_OK) {
            Serial.println("[Alice] HOME_COMPLETE enviado al Central");
        } else {
            Serial.printf("[Alice] Error enviando HOME_COMPLETE: %d\n", result);
        }
    }
}
...
void prepareForNextPulse(uint32_t pulseNum) {
    if (!isHomed) {
        Serial.println("[Alice] ERROR: No se ha realizado homing");
    ...
    if (centralRegistered) {
        ...
        response.pulseNum = pulseNum;
        response.base = baseAlice;
        response.bit = bitAlice;
        response.angle = currentTargetAngle;
        
        esp_err_t result = esp_now_send(centralMAC, (uint8_t*)&response, sizeof(response));
        if (result == ESP_OK) {
            Serial.printf("[Alice] READY enviado - Pulso:%d Base:%d Bit:%d\n", pulseNum, baseAlice, bitAlice);
        } else {
            Serial.printf("[Alice] Error enviando READY: %d\n", result);
        }
    }
}
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
    if (len == sizeof(CommandData)) {
        ...
        if (!centralRegistered) {
            ...
            Serial.print("[Alice] MAC Central registrada: ");
            for (int i = 0; i < 6; i++) {
        ...
        switch (cmd.cmd) {
            case CMD_PING:
                Serial.println("[Alice] PING recibido - Respondiendo PONG");
                {
                    ResponseData response;
                    ...
                    esp_err_t result = esp_now_send(centralMAC, (uint8_t*)&response, sizeof(response));
                    if (result == ESP_OK) {
                        Serial.println("[Alice] PONG enviado exitosamente");
                    } else {
                        Serial.printf("[Alice] ERROR enviando PONG: %d\n", result);
                    }
                }
                break;
            
            case CMD_HOME:
                Serial.println("[Alice] Comando HOME recibido");
                performHoming();
                break;
                
            case CMD_PREPARE_PULSE:
                Serial.printf("[Alice] Comando PREPARE_PULSE recibido - Pulso:%d\n", cmd.pulseNum);
                currentPulseNum = cmd.pulseNum;
                prepareForNextPulse(cmd.pulseNum);
                break;
                
            case CMD_ABORT:
                Serial.println("[Alice] Comando ABORT recibido");
                stepper.stop();
                break;
                
            default:
                Serial.printf("[Alice] Comando desconocido: %d\n", cmd.cmd);
                break;
        }
    } else {
        Serial.printf("[Alice] Mensaje recibido con tamaño incorrecto: %d bytes\n", len);
    }
}
...
void setup() {
    ...
    Serial.println("[Alice] Canal fijado en 13");
    
    // Inicializar ESP-NOW
    if (esp_now_init() != ESP_OK) {
        Serial.println("[Alice] ERROR: esp_now_init() falló");
        return;
    }
    Serial.println("[Alice] ESP-NOW inicializado");
    
    // Registrar callback de recepción
    if (esp_now_register_recv_cb(OnDataRecv) != ESP_OK) {
        Serial.println("[Alice] ERROR: No se pudo registrar callback RX");
        return;
    }
    Serial.println("[Alice] Callback RX registrado");
    ...
    delay(100);
    Serial.println("[Alice] Iniciando driver TMC2130...");
    driver.begin();
    delay(100);
    digitalWrite(ENABLE_PIN, LOW);
    
    if (driver.test_connection()) {
        Serial.println("[Alice] Driver TMC2130 detectado");
        driver.rms_current(stepperCurrent);
        driver.stealthChop(0);
        driver.pwm_autoscale(true);
        driver.microsteps(microsteps);
    } else {
        Serial.println("[Alice] ERROR: No se detectó driver");
    }
    ...
    Serial.println("[Alice] Configuración completa - Esperando comandos del ESP32 Central vía ESP-NOW\n");
}
